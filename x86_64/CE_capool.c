...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 9, 4);
 	...
 	ivoc_help("help ?1 capool /group/neurolab/Neuromorpho_HOC/x86_64/CE_capool.mod\n");
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   Dcai = - fcas * A * ica + ( cainf - cai ) / taucas ;
   }
 return _reset;
}
 static int _ode_matsol1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {
 Dcai = Dcai  / (1. - dt*( ( ( ( - 1.0 ) ) ) / taucas )) ;
 return 0;
}
 /*END CVODE*/
 static int states (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) { {
    cai = cai + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / taucas)))*(- ( ((- fcas)*(A))*(ica) + ( ( cainf ) ) / taucas ) / ( ( ( ( - 1.0) ) ) / taucas ) - cai) ;
   }
  return 0;
}
